...
async changeMaxRecordingDurationSetting(duration: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_max_recording_duration_setting", { duration }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type MaxRecordingDuration = "min_1" | "min_2" | "min_5" | "min_10" | "min_30" | "unlimited"
